...
// tate small numbers
const firstNumbers = JSON.parse(fs.readFileSync(`${__dirname}/lib/tateStuff/firstNumbers.json`, 'utf-8'))
const firstNumbersAdded = sumValues(firstNumbers);
console.log('firstNumbersAdded', firstNumbersAdded);


// tate state
const tateState = JSON.parse(fs.readFileSync(`${__dirname}/lib/tateStuff/tateFile.json`, 'utf-8'))
const tateStateAdded = await sumVault(tateState.vault)
console.log('tateStateAdded', tateStateAdded)


// smartweave state
const smartweaveState = JSON.parse(fs.readFileSync('./smartweaveState.json', 'utf-8'))
const smartweaveStateAdded = await sumVault(smartweaveState.vault)
console.log('smartweaveStateAdded', smartweaveStateAdded)


// tom tool state
const tomToolState = JSON.parse(fs.readFileSync(`${__dirname}/lib/tomTool/tomToolRes.json`, 'utf-8'))
const tomToolStateAdded = await sumVault(tomToolState.vault)
console.log('tomToolStateAdded', tomToolStateAdded)


// PPE state
const ppeState = JSON.parse(fs.readFileSync(`${__dirname}/lib/ppeRecommendation/ppeRecommendation.json`, 'utf-8'))
